...
//
// 该 CDN（chinese-fonts-cdn.deno.dev 及其轮换网关主机，如 cn-font.claude-code-best.win）
// 对 result.css 与 woff2 均校验 Referer：无 Referer 一律 403（响应体「此 URL 不支持直接
// 打开」）。而 Chrome 出于隐私会剥离 chrome-extension:// 页面发出的 Referer，导致 <link>
// 与 fetch 都拿不到字体。
//
// 解决：用 declarativeNetRequest 在网络层为该 CDN 的请求补一个 Referer 头——正好补回被
// 浏览器剥离的那个头，符合服务商「需携带 Referer 访问」的规定。用路径正则匹配（不绑定
// 主机名），以兼容入口 301 重定向到的轮换网关主机。

const RULE_ID = 1001
const FONT_REFERER = "https://chinese-font.netlify.app/"
// 匹配 /packages/<id>/dist/... 这一 CDN 路径结构，任意 https 主机均生效（兼容轮换网关）
